...
def run_model( fn, maps_path, output_filename, ncores, begin_year, end_year ):
	...
	script_path = '/workspace/UA/malindgren/repos/alfresco_postprocessing/bin/alfresco_relative_vegetation_change.py'
	with open( fn, 'w' ) as f:
...
if __name__ == '__main__':
	...
	output_path = '/atlas_scratch/jschroder/ALF_outputs/PP_2017-07-19-09-03_all_polygons/relative_vegetation_change'
	mask_fn = None
	...
	output_filenames = [ os.path.join( output_path, 'alfresco_relative_vegetation_change_'+os.path.basename(sub)+'.tif' ) for sub in sub_dirs ]

	for maps_path, out_fn in zip( maps_paths, output_filenames ):
		for year_range in year_ranges:
			begin_year, end_year = year_range
			output_filename = out_fn.replace('.tif', '_{}_{}.tif'.format(begin_year, end_year))
